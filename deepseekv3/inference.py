...
import tiktoken
from deepseekv3.model import DeepSeek 
from deepseekv3.kv_cache import KVCache
from deepseekv3.config import DeepSeekConfig
...
def generate(model, tokenizer, prompt, max_new_tokens=20, temperature=0.7, top_p=0.9, device='cuda'):
    ...
    # Note: We access model.config for DeepSeek specifics
    
    # DEEPSEEK MLA SPECIAL CASE
    # Unlike Llama/Mixtral where we have N heads, DeepSeek MLA stores a single 
    # compressed latent vector. We treat this as "1 Head" of size "kv_lora_rank" (512).
    head_dim = model.config.kv_lora_rank 
    n_kv_heads = 1 
