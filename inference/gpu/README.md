...
with the fundamentals in place, files 16-19 apply them: a targeted custom kernel for a measured bottleneck, a decode-time kv cache kernel, kernel profiling with nsight, and NCCL/multi-gpu concepts.
...
file 32 specifically builds on file 28's tiling structure: instead of visiting every key block, it skips blocks a query's sparsity pattern excludes, and the lesson is as much about what quality is given up as about the speed gained.
...
- cuda c++ (.cu files) — the actual kernel source for 09, 12, 13, 14, 16, 17, 28, 29, 30, 31 and 32; compiled at runtime through nvrtc and launched through cudarc
