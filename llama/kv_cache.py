...
class KVCache:
    def __init__(self, config):
        ...
        super().__init__()
        cache_shape = (config.max_batch_size, config.max_seq_len, config.n_kv_heads, config.head_dim)
        # We use 'register_buffer' to tell PyTorch:
        # "This is part of the model state, but it is NOT a learnable weight (no gradients)."
        self.register_buffer('k_cache', torch.zeros(cache_shape, device=device))
        # we did not use torch.zeros since pytorch wouldn't move that to GPU as buffer
